...
-- Derived from db/schema.sql, which stays as the readable reference. Migrations
-- are authoritative and forward-only: never edit this file once applied (the
-- runner enforces that with a checksum).
...
-- ---------------------------------------------------------------
-- Sightings — EVERY plate read, not only watchlist hits (invariant 1).
-- The evaluation plate arrives after the vehicle has passed; without
-- complete history the trace is impossible.
-- ---------------------------------------------------------------
...
CREATE TABLE alerts (
    ...
    -- NOTE: db/schema.sql carries
    --     FOREIGN KEY (sighting_id, sighting_ts) REFERENCES sightings(id, ts)
    -- here. TimescaleDB does not support foreign keys that *reference* a
    -- hypertable, so that constraint cannot be created and is deliberately
    -- omitted. Referential integrity for this edge is enforced in the alerting
    -- writer, which only ever inserts an alert for a sighting it has just
    -- written in the same transaction. The composite index below keeps the
    -- join to sightings cheap.
);
...
CREATE INDEX alerts_sighting_idx ON alerts (sighting_id, sighting_ts);
